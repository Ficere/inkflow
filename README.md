...
| **批量工作流** | `generate_batch_cards.py` 可把一批 Markdown 草稿渲染成逐篇文件夹 + 总合集 |
...
```bash
python scripts/generate_batch_cards.py \
  /path/to/drafts \
  /path/to/avatar.png \
  /path/to/output \
...
批量脚本输出的图片、文件名、清单与发布文案均按「个人思考笔记 / 原创笔记」口吻呈现。
...
│   └── generate_batch_cards.py # 批量生成 PNG + 公众号发布文案
├── references/
